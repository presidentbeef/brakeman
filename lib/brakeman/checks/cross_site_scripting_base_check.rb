...
# <%= User.find(:id).name %>
# <%= params[:id] %>
class Brakeman::CrossSiteScriptingBaseCheck < Brakeman::BaseCheck
  ...
  def check_for_immediate_xss exp
    ...
    warn_for_immediate_xss(exp, out)
  end
  ...
  def actually_process_call exp
    ...
    #Ignore safe items
    if ignore_call? target, exp.method
      @matched = false
    elsif set_matched!(exp, target)

...
class Brakeman::CheckQuoteTableName < Brakeman::BaseCheck
  ...
  def run_check
    if (version_between?('2.0.0', '2.3.13') or 
      ...
      if rails_version =~ /^3/
        message = msg("Versions before 3.0.10 have a vulnerability in ", msg_code("quote_table_name"), " (CVE-2011-2930)")
      else
        message = msg("Versions before 2.3.14 have a vulnerability in ", msg_code("quote_table_name"), " (CVE-2011-2930)")
      end
